...
A Revision is a monotonically increasing 128-bit unsigned integer (`u128`) that uniquely identifies a mutation. Revisions
are **system-wide unique** — generated by the rKV instance and shared across all namespaces. A single global counter
ensures no two mutations, regardless of namespace, ever produce the same revision. Every successful `put` returns the new
Revision to the caller. Revisions are displayed as Crockford Base32 encoded strings (e.g., `7z` for revision 255).

History queries use revision indexing: `rev_count(key)` returns the total number of revisions for a key, and
`rev_get(key, index)` retrieves the value at a specific revision index (0 = oldest). CAS operations use revision IDs for
