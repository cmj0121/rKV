...
- `docker compose up` — **default**: current 5-node full-replication (unchanged behavior)
- `docker compose --profile cluster up` — **cluster**: 2 shard groups (2 peers each) + 1 gateway

### Cluster Services

| Service    | Role    | Shard Group | Cluster ID | Port | Namespaces       |
| ---------- | ------- | ----------- | ---------- | ---- | ---------------- |
| `gateway`  | Gateway | —           | —          | 8320 | Routes all       |
