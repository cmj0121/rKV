...
fn fuzz_random_queue_ops() {
    ...
    while Instant::now() < deadline {
        ...
        match op {
            ...
            // pop_message (25%)
            50..75 => {
                let backend_result = rt
                    .block_on(backend.pop_message(name))
                    .unwrap()
                    .map(|(_, body)| body);
...
fn fuzz_fifo_ordering_stress() {
    ...
    while Instant::now() < deadline {
        ...
        match op {
            ...
            60..90 => {
                let got = rt
                    .block_on(backend.pop_message("stress"))
                    .unwrap()
                    .map(|(_, body)| body);
...
#[test]
fn fuzz_ttl_push_does_not_crash() {
    let seed = fuzz_seed();
    let duration = fuzz_duration();
    let mut rng = fastrand::Rng::with_seed(seed);

    eprintln!("rill TTL fuzz: seed={seed}, duration={duration}s");

    let rt = tokio::runtime::Runtime::new().unwrap();
    let backend = embed_backend();

    rt.block_on(backend.create_queue("ttl")).unwrap();

    let deadline = Instant::now() + std::time::Duration::from_secs(duration);
    let mut ops: u64 = 0;

    while Instant::now() < deadline {
        // Random TTL: None, 0s, 1s-3600s
        let ttl = match rng.u32(0..3) {
            0 => None,
            1 => Some(std::time::Duration::from_secs(0)),
            _ => Some(std::time::Duration::from_secs(rng.u64(1..3600))),
        };
        let msg = format!("ttl-{ops}");
        let id = rt.block_on(backend.push_message("ttl", &msg, ttl)).unwrap();
        // ID is always 26-char ULID
        assert_eq!(id.len(), 26, "bad ID length at op={ops}, seed={seed}");
        ops += 1;
    }

    // Verify queue is readable (some messages may have expired with TTL=0)
    let len = rt.block_on(backend.queue_length("ttl")).unwrap();
    eprintln!("rill TTL fuzz: {ops} pushes, {len} remaining, seed={seed}");
}
