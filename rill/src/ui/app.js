...
function addActivity(action, queue, message, id) {
  activityLog.unshift({
    action: action,
    queue: queue,
    message: message,
...
function openPushDialog() {
  ...
  dlg.appendChild(
    el("label", {
      textContent: "TTL (e.g. 30s, 5m, 2h, 1d — blank = no expiry)",
    }),
  );
  ...
  var actions = el("div", { className: "dialog-actions" }, [
    ...
    el("button", {
      ...
      onClick: function () {
        ...
        if (ttlVal) {
          url += "?ttl=" + encodeURIComponent(ttlVal);
        }
        api("POST", url, msg)
          .then(function (r) {
            var id = r.data && r.data.id;
            var suffix = ttlVal ? " (TTL: " + ttlVal + ")" : "";
            toast("Pushed: " + (id || "ok") + suffix, true);
            addActivity("push", state.selectedQueue, msg, id);
            loadMessages();
          })
          .catch(function (e) {
  ...
  api("GET", "/queues/" + encodeURIComponent(state.selectedQueue))
    .then(function (r) {
      ...
      if (msg === null || msg === undefined) {
        ...
        addActivity("pop", state.selectedQueue, "(empty)", null);
      } else {
        toast("Popped: " + (id || "ok"), true);
        addActivity("pop", state.selectedQueue, String(msg), id);
      }
      loadMessages();
    })
    .catch(function (e) {
