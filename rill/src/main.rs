...
/// Parse a human-readable TTL string like `30s`, `5m`, `2h`, `1d`, or plain
/// seconds (`120`).  Matches rKV's `X-RKV-TTL` format.
fn parse_ttl(s: &str) -> Option<std::time::Duration> {
    ...
    Some(std::time::Duration::from_secs(num * unit))
}
...
async fn push_message(
    ...
    validate_queue_name(&name)?;
    let ttl = query.ttl.as_deref().and_then(parse_ttl);
    let id = state
        .backend
...
async fn pop_message(
    ...
    let result = state
        ...
        .map_err(ApiError::Internal)?;
    match result {
        Some((id, body)) => Ok(Json(json!({"id": id, "message": body}))),
        None => Ok(Json(json!({"id": null, "message": null}))),
    }
}
...
mod tests {
    ...
    fn remote_state() -> Arc<AppState> {
        Arc::new(AppState {
            backend: Backend::remote("http://localhost:9999"),
            admin_token: None,
            writer_token: None,
            reader_token: None,
            ui_enabled: false,
    ...
    #[tokio::test]
    async fn push_with_ttl_query_accepted() {
        let app = build_router(open_state());
        ...
        // Message is readable
        let (_, peek_body) = request(&app, "GET", "/queues/ttl/messages", None, None).await;
        assert!(peek_body.contains("expires-later"));
    }

    #[tokio::test]
    async fn push_with_invalid_ttl_query_ignored() {
        let app = build_router(open_state());
        request(&app, "POST", "/queues", None, Some(r#"{"name":"inv"}"#)).await;
        // Non-parseable TTL query param is silently ignored (treated as no TTL)
        let (status, body) = request(
            &app,
            "POST",
            "/queues/inv?ttl=notanumber",
            None,
            Some("msg"),
        )
        .await;
    ...
    #[tokio::test]
    async fn push_with_ttl_suffixes() {
        let app = build_router(open_state());
        request(&app, "POST", "/queues", None, Some(r#"{"name":"ts"}"#)).await;
        // All suffix forms should work
        for ttl in &["30s", "5m", "2h", "1d", "120"] {
            let (status, _) = request(
                &app,
                "POST",
                &format!("/queues/ts?ttl={ttl}"),
                None,
                Some("x"),
            )
            .await;
            assert_eq!(status, StatusCode::OK, "failed for ttl={ttl}");
        }
    }

    #[tokio::test]
    async fn push_response_includes_ulid_id() {
        let app = build_router(open_state());
        request(&app, "POST", "/queues", None, Some(r#"{"name":"uid"}"#)).await;
        let (status, body) = request(&app, "POST", "/queues/uid", None, Some("data")).await;
        assert_eq!(status, StatusCode::OK);
        let parsed: serde_json::Value = serde_json::from_str(&body).unwrap();
        let id = parsed["id"].as_str().unwrap();
        assert_eq!(id.len(), 26, "ULID should be 26 chars: {id}");
        assert!(parsed["pushed"].as_bool().unwrap());
    }

    #[test]
    fn parse_ttl_formats() {
        ...
        assert_eq!(parse_ttl("120"), Some(Duration::from_secs(120)));
        assert_eq!(parse_ttl("notanumber"), None);
